...
import dev.ynki.util.player.AudioUtil;
...
public abstract class Function implements IMinecraft {
    ...
    public void toggle() {
        ...
        String icon = state ? "i" : "k"; // i = включено, k = выключено
        String message = name + " " + (state ? "включена!" : "выключена!");
        NotificationManager.add(Text.literal(message), icon);
    }

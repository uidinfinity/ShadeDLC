...
import dev.ynki.events.impl.player.EventSprint;
import dev.ynki.manager.Manager;
import dev.ynki.mixin.iface.ClientPlayerEntityAccessor;
import dev.ynki.modules.Function;
import dev.ynki.modules.FunctionAnnotation;
import dev.ynki.modules.Type;
import dev.ynki.modules.movement.ElytraTarget;
import dev.ynki.modules.render.littlePet.GhostWolfEntity;
...
import dev.ynki.util.player.AuraUtil;
...
public class AttackAura extends Function {
    ...
    private boolean hasAttackedThisJump = false;
    ...
    public void onEvent(Event event) {
        ...
        if (event instanceof EventUpdate) {
            // Сбрасываем флаг атаки при приземлении
            if (player.isOnGround()) {
                hasAttackedThisJump = false;
            }
    ...
    protected void onDisable() {
        if (target != null && isValidTarget(target)) {
            ...
            } else if (modeName.equals("AiRotation")) {
                // Для AiRotation камера должна спавниться туда, куда была нацелена ротация
        ...
        target = null;
        cpsLimit = System.currentTimeMillis();
        // Сбрасываем переменные AiRotation
        this.isFastikomActive = false;
        this.lastFastikomTime = 0L;
        this.fastikomStartTime = 0L;
        this.fastikomOffsetYaw = 0.0f;
        this.fastikomOffsetPitch = 0.0f;
        this.currentBodyPartIndex = 0;
        this.lastBodyPartChangeTime = 0L;
        this.aiRotationAttackDelay = 0L;
        super.onDisable();
    }
    ...
    // AiRotation variables
    private long lastFastikomTime = 0L;
    private boolean isFastikomActive = false;
    private long fastikomStartTime = 0L;
    private float fastikomOffsetYaw = 0.0f;
    private float fastikomOffsetPitch = 0.0f;
    private int currentBodyPartIndex = 0;
    private long lastBodyPartChangeTime = 0L;
    private long aiRotationAttackDelay = 0L;
    ...
    private void handleAttackAndRotation(LivingEntity t) {
        ...
        if (mode.is("AiRotation")) {
            long currentTimeMs = System.currentTimeMillis();
            boolean canAttackWithDelay = canAttackNow && (this.aiRotationAttackDelay == 0L || currentTimeMs >= this.aiRotationAttackDelay);
            
            if (canAttackWithDelay && passRay && noPotion) {
                // Устанавливаем задержку на следующую атаку (0-15 миллисекунд)
                this.aiRotationAttackDelay = currentTimeMs + random.nextInt(16);
                attackTarget(mc.player);
            }
    ...
    public void attackTarget(PlayerEntity player) {
        boolean elytraFlight = mc.player != null && mc.player.isGliding();

        // Проверяем, не атаковали ли уже в этом прыжке (для элитр ограничение не действует)
        if (!elytraFlight && hasAttackedThisJump) {
            return;
        }

        // ТОЛЬКО КРИТАМИ - атака только в воздухе
        if (!elytraFlight && (mc.player.isOnGround() || mc.player.fallDistance <= 0.0f)) {
            return; // Не атакуем на земле, только в воздухе для критов
        }
        ...
        // Уменьшаем задержку для более быстрых атак
        cpsLimit = System.currentTimeMillis() + 50L;

        mc.interactionManager.attackEntity(player, target);
        mc.player.swingHand(MAIN_HAND);
        
        // Помечаем, что уже ударили в этом прыжке (на элитрах не ограничиваем)
        if (!elytraFlight) {
            hasAttackedThisJump = true;
        }
    ...
    private boolean canAttack() {
        ...
        // Более мягкая проверка кулдауна для быстрых атак
        if (!(mc.player.getMainHandStack().isOf(Items.MACE))
                && mc.player.getAttackCooldownProgress(mc.getRenderTickCounter().getTickDelta(true)) < 0.85F) return false;

        // Убираем проверки на дебаффы - KillAura должна работать даже с плохими эффектами
        // Оставляем только критичные проверки (полет, транспорт, лава)
        boolean restrict = mc.player.isInLava()
                || mc.player.hasVehicle()
                || mc.player.getAbilities().flying;

        boolean elytraFlight = mc.player.isGliding();

        // ВСЕГДА ТОЛЬКО КРИТАМИ - принудительно
        // Атака возможна только в воздухе (криты по ванильной механике)
        if (!restrict) {
            // Если на земле - можно прыгнуть (атака произойдет в воздухе)
            if (mc.player.isOnGround()) {
                return true; // Разрешаем прыжок для следующей атаки
            }
            // Если в воздухе - можно атаковать только если еще не атаковали в этом прыжке
            return !mc.player.isOnGround()
                    && mc.player.fallDistance > 0.0f
                    && (elytraFlight || !hasAttackedThisJump);
        }
        return false;
    }
    ...
    private void aiRotation(LivingEntity entity) {
        if (mc.player == null || entity == null) return;
        
        long currentTime = System.currentTimeMillis();
        
        // Меняем точку прицеливания на теле каждые 200-400мс
        if (currentTime - this.lastBodyPartChangeTime > 200L + random.nextInt(200)) {
            this.currentBodyPartIndex = random.nextInt(5);
            this.lastBodyPartChangeTime = currentTime;
        }
        
        // Выбираем случайную точку на теле
        double bodyPartHeight = 0.0D;
        switch (this.currentBodyPartIndex) {
            case 0: // Голова
                bodyPartHeight = entity.getHeight() * 0.9D;
                break;
            case 1: // Грудь
                bodyPartHeight = entity.getHeight() * 0.7D;
                break;
            case 2: // Торс
                bodyPartHeight = entity.getHeight() * 0.5D;
                break;
            case 3: // Ноги
                bodyPartHeight = entity.getHeight() * 0.3D;
                break;
            case 4: // Низ
                bodyPartHeight = entity.getHeight() * 0.1D;
                break;
        }
        
        // Добавляем небольшой рандомный сдвиг по X и Z
        double randomX = (random.nextDouble() * 0.3 - 0.15);
        double randomZ = (random.nextDouble() * 0.3 - 0.15);
        
        Vec3d targetPos = entity.getPos().add(randomX, bodyPartHeight, randomZ);
        Vec3d eyePos = mc.player.getEyePos();
        ...
        double yaw = (Math.atan2(deltaZ, deltaX) * 180 / Math.PI) - 90.0;
        double pitch = -(Math.atan2(deltaY, hDistance) * 180 / Math.PI);
        
        float yawToTarget = (float) yaw;
        float pitchToTarget = (float) pitch;
        
        // Fastikom логика - иногда убираем прицеливание с hitbox на 3-4 пикселя
        if (!this.isFastikomActive && currentTime - this.lastFastikomTime > 1500L + random.nextInt(2000)) {
            // Начинаем fastikom
            this.isFastikomActive = true;
            this.fastikomStartTime = currentTime;
            this.fastikomOffsetYaw = (random.nextFloat() * 1.0f + 3.0f) * (random.nextBoolean() ? 1.0f : -1.0f);
            this.fastikomOffsetPitch = (random.nextFloat() * 1.0f + 3.0f) * (random.nextBoolean() ? 1.0f : -1.0f);
            this.lastFastikomTime = currentTime;
        }
        
        if (this.isFastikomActive) {
            long fastikomDuration = currentTime - this.fastikomStartTime;
            if (fastikomDuration < 30L) {
                // Убираем прицеливание с hitbox (быстро)
                yawToTarget += this.fastikomOffsetYaw;
                pitchToTarget += this.fastikomOffsetPitch;
            } else if (fastikomDuration < 80L) {
                // Возвращаем обратно на hitbox (плавно)
                float returnProgress = (float)(fastikomDuration - 30L) / 50.0f;
                yawToTarget += this.fastikomOffsetYaw * (1.0f - returnProgress);
                pitchToTarget += this.fastikomOffsetPitch * (1.0f - returnProgress);
            } else {
                // Заканчиваем fastikom
                this.isFastikomActive = false;
            }
        }
        
        // Уменьшенный шум (вместо больших значений, делаем меньше)
        float noiseYaw = (random.nextFloat() * 4.0f - 2.0f);
        float noisePitch = (random.nextFloat() * 4.0f - 2.0f);
        
        float finalYaw = yawToTarget + noiseYaw;
        float finalPitch = MathHelper.clamp(pitchToTarget + noisePitch, -90.0f, 90.0f);
        
        Manager.ROTATION.setSmooth(finalYaw, finalPitch, 0.8f, 60, 90, true);
    }
}
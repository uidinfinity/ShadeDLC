...
@FunctionAnnotation(
        name = "ClickGUI",
        desc = "Управление/Кастомизация GUI",
        type = Type.Render,
...
public class ClickGUI extends Function {
    ...
    // ⬇ КНОПКА ПЕРЕКЛЮЧЕНИЯ НА АЛЬТЕРНАТИВНЫЙ GUI
    public final BooleanSetting alternativeGUI = new BooleanSetting("Альтернативный GUI", false);

    public ClickGUI() {
        addSettings(colorGUI, alpha, blur, blurSetting, strike, filling, rounding, alphaModules, alternativeGUI);
    }
    ...
    public void onEnable() {
        setState(false); // Не включать как обычный модуль

        if (alternativeGUI.get()) {
            // Открываем НОВЫЙ, АЛЬТЕРНАТИВНЫЙ GUI
            mc.setScreen(new dev.ynki.screens.gui.AlternativeGUI());
        } else {
            // Открываем СТАНДАРТНЫЙ GUI
            mc.setScreen(new dev.ynki.screens.dropdown.ClickGUI());
        }
    }
}
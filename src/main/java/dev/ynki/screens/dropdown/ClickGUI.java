package dev.ynki.screens.dropdown;

import dev.ynki.modules.setting.*;
import dev.ynki.screens.dropdown.impl.*;
...
import dev.ynki.manager.themeManager.Style;
import dev.ynki.modules.Function;
import dev.ynki.modules.Type;
import dev.ynki.screens.dropdown.search.SearchState;
...
import static dev.ynki.util.render.RenderUtil.drawBlur;
...
public class ClickGUI extends Screen implements IMinecraft {
    ...
    private float getRounding() {
        return Manager.FUNCTION_MANAGER.clickGUI.rounding.get().floatValue();
    }
    ...
    private void drawDescription(DrawContext ctx, String desc, int startY) {
        ...
        if (Manager.FUNCTION_MANAGER.clickGUI.blur.get() && Manager.FUNCTION_MANAGER.clickGUI.blurSetting.get("Описание")) {
            drawBlur(ctx.getMatrices(), descX - 6, descY - 3.5f, descWidth + 12, descHeight, rounding, 8, -1);
        }
        RenderUtil.drawRoundedRect(ctx.getMatrices(), descX - 6, descY - 3.5f, descWidth + 12, descHeight, rounding, GUI_COLOR.getRGB());
    ...
    private void renderPanel(DrawContext ctx, int x, int y, Type category, int mouseX, int mouseY) {
        ...
        if (Manager.FUNCTION_MANAGER.clickGUI.blur.get() && Manager.FUNCTION_MANAGER.clickGUI.blurSetting.get("Панели")) {
            drawBlur(ctx.getMatrices(), x, y, PANEL_WIDTH, PANEL_HEIGHT, rounding, 8, -1);
        }
        RenderUtil.drawRoundedRect(ctx.getMatrices(), x, y, PANEL_WIDTH, PANEL_HEIGHT, rounding, GUI_COLOR.getRGB());
        ...
        for (Function f : Manager.FUNCTION_MANAGER.getFunctions(category)) {
            ...
            dev.ynki.modules.render.ClickGUI clickGUI = Manager.FUNCTION_MANAGER.clickGUI;
            ...
            if (clickGUI.filling.get()) {
                RenderUtil.rectRGB(ctx.getMatrices(), x + 4, currentY - 1, PANEL_WIDTH - 8, totalHeight - 1, rounding, colorModule2, colorModule2, colorModule2, colorModule2);
            }
            if (clickGUI.strike.get()) {
                RenderUtil.drawRoundedBorder(ctx.getMatrices(), x + 4, currentY - 1, PANEL_WIDTH - 8, totalHeight - 1, rounding, 0f, colorModule2);
            }
    ...
    private void renderScrollbar(DrawContext ctx, int x, int y, Type category, float offset) {
        ...
        RenderUtil.drawRoundedRect(ctx.getMatrices(), scrollbarX, scrollbarY, scrollbarWidth, scrollbarHeight, rounding, scrollbarBgColor);
        ...
        RenderUtil.drawRoundedRect(ctx.getMatrices(), scrollbarX, thumbY, scrollbarWidth, thumbHeight, rounding, thumbColor);
    }
    ...
    private void renderSearchField(DrawContext ctx) {
        ...
        if (clickGUI.blur.get() && clickGUI.blurSetting.get("Поиск")) {
            drawBlur(ctx.getMatrices(), searchX, searchY, searchWidth, SEARCH_HEIGHT, rounding, 12, -1);
        }
        RenderUtil.drawRoundedRect(ctx.getMatrices(), searchX, searchY, searchWidth, SEARCH_HEIGHT, rounding, GUI_COLOR.getRGB());
    ...
    private void renderButtomTheme(DrawContext ctx, double mouseX, double mouseY) {
        ...
        RenderUtil.drawRoundedRect(ctx.getMatrices(), buttonX, buttonY, buttonWidth, buttonHeight, rounding, color);
    ...
    private void renderTheme(DrawContext ctx, int mouseX, int mouseY) {
        ...
        if (Manager.FUNCTION_MANAGER.clickGUI.blur.get() && Manager.FUNCTION_MANAGER.clickGUI.blurSetting.get("Темы")) {
            drawBlur(ctx.getMatrices(), themeX, themeY + offsetY, themeWidth, THEME_HEIGHT, rounding, 12, -1);
        }
        RenderUtil.drawRoundedRect(ctx.getMatrices(), themeX, themeY + offsetY, themeWidth, THEME_HEIGHT, rounding, panelColor);
    ...
    private void renderColorPickers(DrawContext ctx, int x, int y, int mouseX, int mouseY) {
        ...
        if (Manager.FUNCTION_MANAGER.clickGUI.blur.get() && Manager.FUNCTION_MANAGER.clickGUI.blurSetting.get("Создание темы")) {
            RenderUtil.drawBlur(ctx.getMatrices(), 0, 0, panelWidth, panelHeight, rounding, 12, -1);
        }
        RenderUtil.drawRoundedRect(ctx.getMatrices(), 0, 0, panelWidth, panelHeight, rounding, ColorUtil.applyAlpha(baseColor, alphaMult));
        ...
        int closeButtonY = 0;
        RenderUtil.drawRoundedRect(ctx.getMatrices(), closeButtonX, closeButtonY, closeButtonSize, closeButtonSize, rounding, ColorUtil.applyAlpha(Color.WHITE.getRGB(), alphaMult));
        ...
        RenderUtil.drawRoundedRect(ctx.getMatrices(), 14, 39, 56, 8, rounding, ColorUtil.applyAlpha(Color.WHITE.getRGB(), alphaMult));
    ...
    private void renderExpiryText(DrawContext context) {
        ...
        FontUtils.durman[18].drawLeftAligned(context.getMatrices(), "Окончание подписки - " + text, margin + 4, screenHeight - textHeight - margin, Color.WHITE.getRGB());
    }

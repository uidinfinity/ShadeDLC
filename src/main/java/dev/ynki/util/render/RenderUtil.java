...
public class RenderUtil implements IMinecraft {
    ...
    public static void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, int color) {
        drawLiquidGlass(matrices, x, y, width, height, rounding, 1.0f, 8.0f, 16.0f, 4.0f, color);
    }

    public static void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float smoothness, float glassDirection, float glassQuality, float glassSize, int color) {
        final SimpleFramebuffer fbo = TEMP_FBO_SUPPLIER.get();
        final Framebuffer mainFbo = getMainFbo();
        ...
        ShaderProgram shader = RenderSystem.setShader(ResourceProvider.LIQUID_GLASS_SHADER_KEY);
        RenderSystem.setShaderTexture(0, fbo.getColorAttachment());
    ...
    public static void drawLiquidGlass(MatrixStack matrices, float x, float y, float width, float height, Vector4f rounding, float smoothness, float glassDirection, float glassQuality, float glassSize, ColorRGBA color) {
        matrices.push();
        Matrix4f matrix4f = matrices.peek().getPositionMatrix();
        Framebuffer screenFBO = mc.getFramebuffer();
        int screenTexture = screenFBO.getColorAttachment();
        ...
        int b = color.getBlue();
        int a = color.getAlpha();
        builder.vertex(matrix4f, x, y, 0f).color(r, g, b, a);
        builder.vertex(matrix4f, x, y + height, 0f).color(r, g, b, a);
        builder.vertex(matrix4f, x + width, y + height, 0f).color(r, g, b, a);
        builder.vertex(matrix4f, x + width, y, 0f).color(r, g, b, a);
        RenderUtil.render3D.endBuilding(builder);
        RenderSystem.setShaderTexture(0, 0);
        RenderUtil.disableRender();
        RenderSystem.enableDepthTest();
    ...
    public static void drawTexture(MatrixStack matrices, Object texture, float x, float y, float width, float height, float rounding, int color) {
        ...
        if (texture instanceof String path) {
            textureId = Identifier.of("velyasik", path);
